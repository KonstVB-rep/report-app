...
import type { EventCalendarSchema } from "../model/schema"
...
export const handleEventClick = (
  ...
  if (event.end) {
    ...
    form.setValue("endDateEvent", event.start ? new Date(event.start) : new Date())
  }
  ...
  setEditingId(event.id)
  const popover = document.querySelector(".fc-popover.fc-more-popover.fc-day") as HTMLElement
  if (popover) {
