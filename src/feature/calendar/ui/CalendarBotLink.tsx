  ...
  const handleUnsubscribeChatBot = async (chatName: string, chatId: string) => {
    ...
    try {
      ...
      if (chatId) {
        ...
        await sendNotification("Вы успешно отписались от уведомлений", chatId, bot.botName);
      }
      ...
      // Показываем ошибку
      TOAST.ERROR("Не удалось отписаться от уведомлений. Попробуйте еще раз или позже.");
    ...
    <>
      <div className="flex flex-col items-center space-x-2">
        ...
        <Label htmlFor="calendar-bot" className="flex gap-1">
          <span className="text-xs text-stone-600">{isChecked ? "Включены" : "Выключены"}</span>
        </Label>
      </div>
    </>
  );
