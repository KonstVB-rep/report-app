...
const useCalendarMobile = () => {
  ...
  const futureEvents = useMemo(() => {
    ...
    return events.filter(
      (event) => new Date(event.start).getTime() >= todayTime,
    );
  }, [events, todayTime]);
  ...
  const eventsDate = useMemo(() => {
    ...
    return futureEvents.filter(
      (event) => getStartOfDay(event.start) === selectedTime,
    );
  ...
  const handleSelect = useCallback(
    (date: Date | undefined) => {
      ...
      const hasEvents = futureEvents.some(
        (event) => getStartOfDay(event.start) === dateStart,
      );
