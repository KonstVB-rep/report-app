...
const DepartmentPersonsList = ({ item }: { item: DepartmentListItemType }) => {
  ...
  return (
    <Collapsible open={open} onOpenChange={() => setOpen((prev) => !prev)} asChild>
      <SidebarMenuItem>
        <SidebarMenuButton
          asChild
          tooltip={item.title}
                ...
                {item.items.map((user) => {
                  ...
                  return (
                    <SidebarMenuItem key={user.id}>
                      ...
                      >
                        <Accordion type="single" collapsible className="w-full !pr-1">
                          <AccordionItem value="item-1" className="group/item w-full border-none">
                            <AccordionTrigger className="mr-[2px] w-full py-1 hover:no-underline">
                              <div className="relative flex flex-col gap-[2px]">
                                <span className="font-semibold capitalize">
                                  {user.username}
