...
import { Check, ChevronDown, ListFilterPlus, Settings2, X } from "lucide-react";
...
const FiltersManagment = ({
  ...
  const [selectedFilterNameFromDb, setSelectedFilterNameFromDb] = useState<
    string | null
  >(null);
  ...
  const handleClearFilters = () => {
    setColumnFilters([]);
    setColumnVisibility({});
    setSelectedFilterNameFromDb(null);
  };
      ...
      <div className="flex items-center gap-2">
        {searchParams.size > 0 && (
          ...
          <HoverCardComponent title={<ListFilterPlus />}>
            <RadioGroup defaultValue="filters" className="p-2">
              {userFiltersData.map((item) => {
                return (
                  <div key={item.id} className="flex items-center space-x-2">
                    <RadioGroupItem value={item.filterName} id={item.id} defaultChecked={item.isActive} onClick={() => filterSelect(item.filterValue)}/>
                    <Label htmlFor={item.id} className="first-letter:uppercase">{item.filterName}</Label>
                  </div>
                );
              })}
            </RadioGroup>
