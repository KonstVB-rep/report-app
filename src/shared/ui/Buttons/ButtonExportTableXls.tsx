...
import { DealTypeLabels, FormatedParamsType, FormatedParamsTypeKey } from "@/entities/deal/lib/constants";
...
import { DealBase } from "../Table/model/types";
...
const handleExport = async <TData,>(
  ...
  const { downloadToExcel } = await import("../Table/excel/downLoadToExcel");
...
const ButtonExportTableXls = <T extends DealBase>({
  ...
  return (
    <ProtectedByPermissions permissionArr={[PermissionEnum.DOWNLOAD_REPORTS]}>
      <Button
        variant={"ghost"}
        onClick={() => handleExport<T>(table, columns, FormatedParamsType[dealType as FormatedParamsTypeKey ])}
