...
async function sendDailyNotifications(
  ...
  for (const item of items) {
    ...
    try {
      ...
      logs.push({ type, chatId, itemName: item.nameDeal, status: "sent" });
      console.log(
        `✅ [${type}] Отправлено: ${item.nameDeal} (Chat: ${chatId})`,
      );
    } catch (error: unknown) {
...
export async function GET() {
  ...
  const moscowNow = new Date(
    startTime.toLocaleString("en-US", { timeZone: "Europe/Moscow" }),
  );
  ...
  try {
    ...
    for (const chat of allChats) {
      ...
      const upcomingEvents = events.filter((event) => {
        ...
        return (
          windows.some((w) => nowTime >= w.start && nowTime <= w.end) &&
          nowTime <= eventTime
        );
      });
    ...
    if (moscowNow.getHours() === 9 && moscowNow.getMinutes() <= 30) {
      ...
      const activeChats = allChats.filter(
        (c) => c.isActive && c.chatId && c.userId,
      );
      const userIds = activeChats.map((c) => c.userId);
