...
export async function isUserHasPermissionByRole(
  ...
  if ((department && department.directorId === user.id) || userWithPermissions?.role === "ADMIN") {
    return true
  }
  ...
  const permissionsUser = userWithPermissions.permissions.map((p) => p.permission.name)
