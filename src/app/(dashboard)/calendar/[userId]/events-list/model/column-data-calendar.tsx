...
export const columnsDataCalendar: ColumnDef<EventInputType, unknown>[] = [
    {
        ...
        cell: (info: CellContext<EventInputType, unknown>) => {
              const {row} = info;
              if(row.original.start.toLocaleDateString("ru-RU") === row.original.end.toLocaleDateString("ru-RU")){
      ...
      },
      {
        ...
        enableHiding: false,
        enableSorting: false,
        accessorFn: (row: EventInputType) => <p className="break-all">{row.title}</p>,
      },
]