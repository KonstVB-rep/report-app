...
export const handleСalculationToExcel = (dataOffer: DataOffer, image: string) => {
  const exportPromise: Promise<DataOffer> = new Promise((resolve) => {
    const worker = new Worker(new URL("../workers/exportCalculation.worker.ts", import.meta.url))
