...
import { SerializedEquipmentItem } from "../lib/types";
        ...
        set((state) => {
          ...
          newItems.forEach((newItem) => {
            ...
            if (existingRow) {
              ...
              existingRow.count =
                (existingRow.count || 0) + (newItem.count || 1);
          ...
          state.data.parts.forEach((part) => {
            part.sections.forEach((sec) => {
              const rowIndex = sec.rows.findIndex(
                (r) => r.rowId === updatedItem.rowId,
              );
              if (rowIndex !== -1) {
                const total =
                  Number(updatedItem.price) * Number(updatedItem.count || 0);
                const purchase =
                  Number(updatedItem.purchasePrice || 0) *
                  Number(updatedItem.count || 0);
...
export const updatePartTitle = (pId: string, val: string) =>
  act().updatePartTitle(pId, val);
...
export const removeRow = (pId: string, sId: string, rId: string) =>
  act().removeRow(pId, sId, rId);
export const removePart = (pId: string) => act().removePart(pId);
export const removeSection = (pId: string, sId: string) =>
  act().removeSection(pId, sId);
